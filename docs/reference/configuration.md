...
| `stageStrategy` | `gameCurrentOrLast` 跟随游戏、`rememberedRegular` 跟随常规关卡（开战前识别，记录用于恢复）、`fixed` 固定关卡 |
| `fallbackStage` | 可选常规兜底关卡，留空关闭；开战前检查完成但没有可用目标，或选关失败且尚未开战时尝试一次，不覆盖恢复记录 |
