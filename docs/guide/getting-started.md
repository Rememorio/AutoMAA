...
全局自动更新与方案的“运行前热更新 MAA 资源”用途不同：前者每 24 小时最多通过稳定通道启动一次核心与随包基础资源维护，并避让正在运行或 90 分钟内即将运行的方案；手动维护和空闲自动维护遇到可识别的临时网络错误时会自动重试一次。当前安装为尚未进入稳定通道的 MaaCore Beta 时，只检查稳定版清单并保留现有版本，不会下载较旧稳定包或覆盖其资源。后者在对应方案启动前获取可独立热更新的识别资源。两类更新都会先在隔离目录中下载候选组件，再由 MaaCore 实际加载全部资源层；通过后才启用。验证失败不会改变当前安装，也不会自动回退资源或静默改用预发布版本。两者都不会下载或更新游戏包体。
