...
- “检测环境”能够显示 maa-cli 与 MaaCore 版本；需要时使用“更新核心与基础资源”和“热更新识别资源”，也可以选择在 AutoMAA 打开且空闲时自动更新稳定版核心与基础资源；
- 游戏客户端已经手动完成包体更新、数据下载、协议确认和登录。

全局自动更新与方案的“运行前热更新 MAA 资源”用途不同：前者每 24 小时最多通过稳定通道启动一次核心与随包基础资源维护，并避让正在运行或 90 分钟内即将运行的方案；手动维护和空闲自动维护遇到可识别的临时网络错误时会自动重试一次。后者在对应方案启动前获取可独立热更新的识别资源。两者都不会下载或更新游戏包体。
