...
- “检测环境”能够显示 maa-cli 与 MaaCore 版本；“更新 MAA”更新引擎及配套识别数据，“仅更新识别数据”只同步识别规则与图片增量；
- 游戏客户端已经手动完成包体更新、数据下载、协议确认和登录。

“空闲时自动更新 MAA”每 24 小时最多尝试一次稳定版，并避让运行中的方案和 90 分钟内的定时任务。方案的“运行前更新识别数据”只在该方案启动前同步数据，两者共用更新与校验流程。识别数据更新上限 3 分钟，完整更新上限 15 分钟；临时网络错误最多重试一次，计入同一上限。自动和手动更新均可取消，失败或取消保留当前安装。更新范围、Beta 与兼容性处理见[更新与人工处理](../troubleshooting/game-update)。
