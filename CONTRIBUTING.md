...
请从 [Issue 入口](https://github.com/Rememorio/AutoMAA/issues/new/choose) 选择最接近的表单：

- Bug 报告需要可复现步骤、实际与预期行为，以及“关于 AutoMAA”复制的版本与环境信息；
- 功能建议先说明需要解决的通用问题和理想结果，具体实现方案可以留给讨论；
- 使用问题请说明已经查阅的文档和尝试过的方法，避免维护者重复猜测本机环境。

画面识别、关卡导航和干员操作属于 MAA / MaaCore；请向上游反馈。可能泄露账号信息、连接错误对象或绕过安全边界的问题不要创建公开 Issue，应使用 GitHub 的私密安全报告。所有公开文字、截图和附件都必须移除账号片段、手机号、邮箱、密码、验证码、令牌和私人路径。
