...
常规换班由所安装的 MaaCore 计算干员组合。AutoMAA 暂未提供跨设施组合和菲亚梅塔恢复的专用参数，相关选项遵循 MaaCore 默认值，详见[基建说明](https://rememorio.github.io/AutoMAA/tasks/infrast)。
...
- 活动记录可按方案、客户端、账号、任务或详情搜索；搜索和“需留意”筛选在切换页面时保留，退出 App 后清空。长详情可展开、换行阅读并复制。
...
安装完成后，可以在“全局设置”管理两类更新：

| 入口 | 更新内容 |
| --- | --- |
| AutoMAA 更新 | 界面与工作流编排功能。正式 Release 下载完成后，仍需确认“重启并立即更新” |
| 更新 MAA | 稳定版 MAA 引擎（MaaCore）、随包基础识别数据，以及单独发布的最新识别数据 |
| 仅更新识别数据 | 界面、关卡和活动的识别规则与图片增量（热更新），不更换 MAA 引擎 |

识别数据由随引擎发布的基础数据和后续增量共同组成；完整更新已包含识别数据同步，不需要再点一遍“仅更新识别数据”。`maa-cli` 是负责调用和管理 MAA 的命令行工具，需要通过原安装方式单独升级。游戏包体和游戏内数据下载仍需用户手动处理。

AutoMAA 的“更新内容”在检查完成、下载、等待安装和失败后都可查看，开启自动下载也不会隐藏说明。详情按版本显示本次升级范围内的变化；“关于 AutoMAA → 本版本更新内容”可随时回看，当前版本说明随 App 提供，支持离线阅读。

MAA 更新详情记录实际启用前后的引擎版本、基础识别数据时间和识别数据仓库修订，并可读取对应版本的上游发布说明、比较资源提交。设置中的“查看 MAA 稳定版发布说明”可在更新前阅读；Beta 说明位于“更新 MAA”菜单。上游说明可能包含其他 GUI 专属变化，不代表 AutoMAA 已支持全部功能；缺少版本或修订信息时会明确标注。说明读取失败可单独重试，不影响更新操作；历史更新详情保留在活动记录中。

自动与手动更新使用相同的超时和取消规则：版本检查上限 30 秒，识别数据更新上限 3 分钟，完整 MAA 更新或 AutoMAA 下载与校验上限 15 分钟。临时网络错误最多重试一次，重试与校验计入同一上限；上游可能因网络或校验错误提前退出。更新过程中显示当前阶段、已用时间和“取消更新”，也可按 `Command-.`；取消后先终止下载进程、清理临时文件，再恢复操作。取消 AutoMAA 下载后，本次打开期间不会自行重新下载，仍可手动继续。已确认重启后的 App 替换由更新器完成。

开启“空闲时自动更新 MAA”后，AutoMAA 打开期间每 24 小时最多尝试一次稳定版，避让运行中的方案和 90 分钟内的定时任务；关闭开关只影响后续自动更新。当前安装为尚未进入稳定通道的 MaaCore Beta 时，只检查稳定版清单并保留现有版本。方案的“运行前更新识别数据”独立控制是否在启动前同步识别数据；下载失败会校验本地数据，只有可用时才继续，取消则停止整个方案。

MAA 引擎与识别数据先写入隔离候选目录，再由对应 MaaCore 在独立进程中按实际顺序加载全部资源层，通过后才启用；失败或取消保留当前安装。不兼容时可手动确认尝试 Beta，或等待稳定版修复，AutoMAA 不会静默切换 Beta 或回退资源。AutoMAA 本体更新只接受正式 Release，替换前校验大小、SHA-256、Bundle ID、版本、架构和代码签名，失败时恢复旧 App。后台定时 Runner 不会自行更新 AutoMAA 本体或 MAA 引擎。
