...
5. 公招、理智、奖励、`Mall` 信用购物，以及 `Infrast mode = 20000` 仅收菜与 `mode = 0` 完整换班的实际参数；
6. 多方案手动运行、独立定时和“方案 + 账号 + 步骤”断点隔离；
7. 无人机不同用途至少验证生成配置，常用用途做一次真机验证；
8. 运行中安全停止、当天断点和重新执行；
9. 游戏缺失、账号匹配失败、端口占用、网络失败、维护、强制更新和数据包下载等边界；
10. 深浅色、较窄窗口、VoiceOver 标签和“减少动态效果”。
